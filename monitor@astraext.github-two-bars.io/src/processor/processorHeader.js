...
import MemoryBars from '../memory/memoryBars.js';
...
export default GObject.registerClass(class ProcessorHeader extends Header {
    ...
    buildBars() {
        ...
        if (this.memBar) {
            this.remove_child(this.memBar);
            Config.clear(this.memBar);
            Utils.memoryMonitor.unlisten(this.memBar);
            this.memBar.destroy();
        }

        this.procBars = new ProcessorBars({
            numBars: numBars,
            header: true,
            layout: 'horizontal',
            mini: true,
        ...
        this.bars.add_child(this.procBars);
        this.bars.add_child(this.memBar);

        Config.bind('memory-header-bars', this.memBar, 'visible', Gio.SettingsBindFlags.GET);
        Utils.memoryMonitor.listen(this.memBar, 'memoryUsage', this.updateMemBars.bind(this));

        Config.bind('processor-header-bars', this.procBars, 'visible', Gio.SettingsBindFlags.GET);

        if (perCoreBars) {
            Utils.processorMonitor.listen(this.procBars, 'cpuCoresUsage', this.updateBarsCores.bind(this));
        }
        else {
            Utils.processorMonitor.listen(this.procBars, 'cpuUsage', this.updateBars.bind(this));
        }
    }
    updateBarsCores() {
        if (!this.visible)
        ...
        if (!usage || !Array.isArray(usage) || usage.length < cores)
            this.procBars.setUsage([]);
        else
            this.procBars.setUsage(usage);
    }
    updateBars() {
        if (!this.visible)
            return;
    ...
    updateMemBars() {
        ...
        if (!usage || !usage.total || isNaN(usage.total))
            this.memBar.setUsage([]);
        else
            this.memBar.setUsage([usage]);
    }
    ...
    buildFrequency() {
        ...
        Config.connect(this.frequency, 'changed::processor-header-frequency-mode', updateFrequencyMode.bind(this));
        updateFrequencyMode();
        const setupFrequency = () => {
            ...
            if (enabled) {
                Utils.processorMonitor.listen(this.frequency, 'cpuCoresFrequency', this.updateFrequency.bind(this));
            }
            else {
                Utils.processorMonitor.unlisten(this.frequency);
            }
        };
        Config.connect(this.frequency, 'changed::processor-header-frequency', setupFrequency.bind(this));
        setupFrequency();
    }
    updateFrequency() {
        if (!this.visible)
            return;
        if (!Config.get_boolean('processor-header-frequency'))
            return;
    ...
    destroy() {
        ...
        if (this.procBars) {
            Config.clear(this.procBars);
            Utils.processorMonitor.unlisten(this.procBars);
        }
        if (this.graph) {
            Config.clear(this.graph);
            Utils.processorMonitor.unlisten(this.graph);
